...
面向低压电器 / 小型断路器生产企业的「AI 询价 → 报价辅助」销售演示。当前公开地址规划为：

```text
https://demo.coreselflab.com/electrical-rfq
```
...
VITE_DEMO_BASE_URL=https://demo.coreselflab.com
```
...
如果当前 Formspree 套餐支持 Domain Restriction，部署完成后将允许来源限制为：

```text
demo.coreselflab.com
```
...
4. 添加自定义域名 `demo.coreselflab.com`。
5. Vercel 会显示当前项目实际需要配置的 CNAME 或其他 DNS Record。
6. 登录腾讯云 DNSPod / DNS 解析控制台，增加主机记录 `demo`。
7. 记录类型和记录值以 Vercel Dashboard 当时显示的要求为准，不要根据代码猜测固定值。
8. 等待 DNS 和 HTTPS 生效后，访问 `https://demo.coreselflab.com/electrical-rfq`。
...
表单会提交企业名称、称呼、联系方式、可选邮箱、主营产品、每日询价量、同意状态、页面 URL、来源参数、referrer、提交时间和 honeypot 字段。URL 中的 `company` 会映射为 `targetCompany`，不会公开显示在页面上：

```text
https://demo.coreselflab.com/electrical-rfq?company=浙江XX电气&campaign=breaker-01&source=email&medium=cold-outreach
```
...
```text
https://demo.coreselflab.com/electrical-rfq?company=浙江测试电气&campaign=test01&source=email&medium=cold-outreach
```

完整走完 Demo，打开「用我司历史询价测试」，填写：
